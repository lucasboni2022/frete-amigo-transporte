...
function SignupForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) {
      const msg = error.message.toLowerCase().includes("password is known to be weak")
        ? "A senha é conhecida por ser fraca e fácil de adivinhar. Escolha uma senha diferente. Exemplo: Frete@2024"
        : error.message;
      toast.error(msg);
